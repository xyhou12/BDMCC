...
% [x,PS] = mapstd(x);
x = x;
near_end = [zeros(1,40000) x(1,40001:60000) zeros(1,60000) x(1,40001:60000) zeros(1,20000)];%%½ü¶ËÓïÒô
% near_end = near_end;
iter = length(x);
...
for n = L+M+1:iter       
    ...
    e_x = x(n) - x_a'*a_hat(:,n);
    K = exp(-e_x^2/sigma_z_n/delta1^2)/exp(-norm(e_a,2)^2/sigma_a/delta1^2);
    alpha = K*sigma_a/(sigma_a*norm(x_a,2)^2*K + sigma_z_n);
...
%     sigma_v_n = q*sigma_v;
    ...
    e_w =  w(:,n) - w(:,n-1);
    K = exp(-e_hat^2/sigma_v_n/delta^2)/exp(-norm(e_w,2)^2/beta/delta^2);
    alpha = K*beta/(beta*norm(x_hat,2)^2*K + sigma_v_n);
...
%     NMSD(n) = NMSD(n) + (norm(w(:,n+1) - h)/norm(h))^2;
    ...
    if n <= iter/2
        NMSD(n) = NMSD(n) + (norm(w(:,n+1) - h)/norm(h))^2;
        ERLE(n) = ERLE(n) + (abs(d_hat)/(e_hat))^2;
    else
...
% figure 
% plot(10*log10(ERLE),'-','linewidth',1.5)
% A = 10*log10(ERLE);
% mean(A)
%%ERLE
